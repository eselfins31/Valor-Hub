...
-- Point BASE at Arsenal folder so we can use src/*.lua paths
local BASE = "https://raw.githubusercontent.com/eselfins31/Valor-Hub/main/Arsenal"

local function notifyErr(msg)
    pcall(function()
        Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = msg, Duration = 6 })
    end)
end
...
-- Load modules from Arsenal/src with safe fallbacks
...
-- Start All/Stop All removed per request; use individual toggles
...
HomeTab:CreateButton({
    ...
    Callback = function()
        ...
        if not ok then
            Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = "Rejoin failed: " .. tostring(err), Duration = 6 })
        end
    end
})
HomeTab:CreateButton({
    ...
    Callback = function()
        ...
        if not ok or not res then
            Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = "Server list fetch failed", Duration = 6 })
            return
        end
        local data = nil
        pcall(function() data = HttpService:JSONDecode(res) end)
        if not data or not data.data then
            Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = "Server list decode failed", Duration = 6 })
            return
        end
        local target = nil
        ...
        if target then
            ...
            if not ok2 then
                Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = "Server hop failed: " .. tostring(err2), Duration = 6 })
            end
        else
            Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = "No suitable server found", Duration = 6 })
        end
    end
})
...
local function bindDropdown(tab, label, stateKey)
    tab:CreateDropdown({
        ...
        Callback = function(opt)
            ...
            Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = label .. " set to " .. tostring(opt), Duration = 3 })
        end
    })
end
...
local function setupAutoReinject()
    ...
    if q then
        ...
        if not ok then
            Rayfield:Notify({ Title = "Valor Hub - Arsenal", Content = "queue_on_teleport failed", Duration = 6 })
        end
